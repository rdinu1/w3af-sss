...
class OpenBSD5(Platform):
    ...
    def os_package_is_installed(package_name):
        ...
        command1 = ['pkg_info']
        command2 = ['grep', '^'+package_name]

        try:
            
            pkg_info_result = subprocess.Popen(command1, stdout=subprocess.PIPE, shell=False)
            grep_result = subprocess.Popen(command2, stdin=pkg_info_result.stdout, stdout=subprocess.PIPE, shell=False)

            pkg_info_result.stdout.close()
            pkg_info_output = grep_result.communicate()[0]
